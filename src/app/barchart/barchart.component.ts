...
export class BarChartComponent implements AfterViewInit {
  ...
  createChart() {
    ...
    series.columns.template.adapters.add(
      'fill',
      function (this: am5xy.ColumnSeries, fill, target) {
        return (
          this.chart?.get('colors')?.getIndex(series.columns.indexOf(target)) ??
          fill
        );
      }
    );
    ...
    series.columns.template.adapters.add(
      'stroke',
      function (this: am5xy.ColumnSeries, stroke, target) {
        return (
          this.chart?.get('colors')?.getIndex(series.columns.indexOf(target)) ??
          stroke
        );
    ...
    let label = this.chart.plotContainer.children.push(
      am5.Label.new(root, {
        text: '2002',
        fontSize: '8em',
        opacity: 0.2,
    ...
    function getSeriesItem(category: any) {
      ...
      return undefined; // Return undefined if no matching dataItem is found
  }
    ...
    function updateData(this: any) {
      ...
      if (allData[year]) {
        ...
        am5.array.each(series.dataItems, function (dataItem) {
          let category: any = dataItem.get('categoryY');
